...
const CivicsFlashcardGame = () => {
  ...
  const filteredQuestions: CivicsQuestion[] = categoryFilter === "All" 
    ? civicsQuestions 
    : civicsQuestions.filter(q => q.category === categoryFilter);

  // Make sure filteredQuestions is not empty and handle the case when it is
  const currentCardIndex_safe = filteredQuestions.length > 0 ? currentCardIndex % filteredQuestions.length : 0;
  const currentCard: CivicsQuestion | undefined = filteredQuestions.length > 0 ? filteredQuestions[currentCardIndex_safe] : undefined;
        ...
        {hearts > 0 ? (
          <div className="w-full max-w-md">
            ...
            {currentCard ? (
              ...
              <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-4 p-6">
                <h2 className="text-xl font-semibold text-center mb-4">No questions available for this category</h2>
                <p className="text-center">Please select a different category</p>
              </div>
            )}
          </div>
        ) : (
